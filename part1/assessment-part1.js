...
Vehicle.prototype.drive = function() {this.gasRemaining -= 25};
...
String.prototype.grammarPolice = function() {
  var strWordArray = this.toLowerCase().split(' ');
  var newArr = [];
  for (i = 0; i < strWordArray.length; i++) {
    if (isNaN(strWordArray[i])) {
      newArr.push(strWordArray[i].charAt(0).toUpperCase() + strWordArray[i].slice(1))
    } else {
      newArr.push(strWordArray[i])
    }
  }
  return newArr.join(' ')
}
